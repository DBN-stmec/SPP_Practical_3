...
cudaError_t setupCudaMemory(SplitScene* scene, const int width, const int height, const int facecnt, float* spheres,
  ...
  // TODO Aufgabe 1)
  cudaStatus =cudaMalloc((void**)&result_Buffer,(size * sizeof(float4)));  // cudaStatus indicates whether Malloc was successful or not

  if (cudaStatus != cudaSuccess) {
    fprintf(stderr, "cudaMalloc for result_Buffer failed!");
    return cudaStatus;
  }
  cudaStatus = cudaMemset(result_Buffer, 0, (size * sizeof(float4)));
  if (cudaStatus != cudaSuccess) {
    fprintf(stderr, "cudaMemset for result_Buffer failed!");
    return cudaStatus;
  }
  // Buffer for vertices
  ...
  if (cudaStatus != cudaSuccess) {
    fprintf(stderr, "cudaMalloc for vertices_Buffer failed!");
    return cudaStatus;
  }
  cudaStatus = cudaMemcpy(vertices_Buffer, vertices, scene->vtxcnt * sizeof(float4), cudaMemcpyHostToDevice);
  if (cudaStatus != cudaSuccess) {
    fprintf(stderr, "cudaMemcpy for vertices_Buffer failed!");
    return cudaStatus;
  }
  // Buffer for camera
  float camera[8];
  ...
  if (cudaStatus != cudaSuccess) {
    fprintf(stderr, "cudaMemcpyToSymbol for camera_Buffer failed!");
    return cudaStatus;
  }
  ...
  cudaStatus = cudaMemcpyToSymbol(lightdat_Buffer, (void*)lightdat, 12 * sizeof(float));
  if (cudaStatus != cudaSuccess) {
    fprintf(stderr, "cudaMemcpyToSymbol for lightdat_Buffer failed!");
    return cudaStatus;
  }
  // TODO Aufgabe 3)
...
__device__ float4 normalizeDirection(float4 d) {
  float4 nd;
  float length = sqrtf(d.x * d.x + d.y * d.y + d.z * d.z);  //+ d.w * d.w);
  nd.x         = d.x / length;
  nd.y         = d.y / length;
  nd.z         = d.z / length;
  nd.w         = 0.0f;
...
__device__ float4 reflect3DLA(float4 l, float4 n) {
  ...
  // TODO Aufgabe 7
  float scalar;
  scalar = (n.x * l.x + n.y * l.y + n.z * l.z);
  r.x    = (2.0f * scalar * n.x) - l.x;
  r.y    = (2.0f * scalar * n.y) - l.y;
  r.z    = (2.0f * scalar * n.z) - l.z;
  r.w    = 0.0f;
  return r;
}
...
__device__ bool rayIntersectsSphere(float4 s, float4 r, float4 o) {
  // TODO Aufgabe 8
  float beta, gamma;
  beta  = (r.x * (o.x - s.x)) + (r.y * (o.y - s.y)) + (r.z * (o.z - s.z));
  gamma = ((o.x - s.x) * (o.x - s.x) + (o.y - s.y) * (o.y - s.y) + (o.z - s.z) * (o.z - s.z)) - (s.w * s.w);
  if ((beta * beta) < gamma) {    //tests if root is negative, if it is -> no solution/complex solution
    return false;
  }
  return true;
}
...
__global__ void traceLA(float4* output, const int2 resinfo, float4* spheres, int4* faces, float4* vertices,
  ...
  // blockId = index of block in the grid ; blockDim = size of block ; threadId = index of thread in the block

  // TODO Aufgabe 5):

  int globalIdx = blockIdx.x * blockDim.x + threadIdx.x;  // blockId = index of block in the grid ; blockDim = size of block
                                                          // ; threadId = index of thread in the block
  int globalIdy = blockIdx.y * blockDim.y + threadIdx.y;
  int globalIdSerial = globalIdy * resinfo.x + globalIdx;
  if (globalIdx >= resinfo.x || globalIdy >= resinfo.y) { //pixels out of imagebounds not calculated because of wrongfully set grid- and blocksizes 
...
cudaError_t traceWithCuda(float* result, const int blocks_x, const int blocks_y, SplitScene* scene, const int width,
  ...
  cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, traceLA);
  cout << "minGridsize:" << minGridSize << endl;
  cout << "blocksize:" << blockSize << endl;
  */
  dim3 threads(32, 28);                              // Output from Occupancy Function 896 -> 32 * 28 ; blockSize : number of threads
  dim3 grid(width / threads.x, height / threads.y);  // Output from Occupany Function: 80  ; GridSize : number of blocks
  ...
  // TODO Aufgabe 4)
  cudaStatus = cudaMemcpy(result, result_Buffer, (height * width) * sizeof(float4), cudaMemcpyDeviceToHost);
  if (cudaStatus != cudaSuccess) {
    fprintf(stderr, "cudaMemcpy to result failed!");
    return cudaStatus;
  }
